  ...
  const fetchTokenPrices = async () => {
    try {
      ...
      setTokenPrices({
        ...
        LNRA: "$0.0234" // Залишаємо статичну ціну для LNRA
      });
    } catch (error) {
      ...
      <div className="container mx-auto px-4 py-4">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div className="flex gap-2 sm:gap-4 overflow-x-auto pb-2 sm:pb-0">
            <button
              ...
              className={`flex items-center gap-2 sm:gap-3 px-3 sm:px-6 py-2 sm:py-3 rounded-xl border-2 transition-all duration-200 flex-shrink-0 ${
                activeTab === 'btc'
            ...
            {/* Timer */}
            <div className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-gray-50 rounded-full border border-gray-200 self-center sm:self-auto">
              <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-gray-600" />
              <span className="text-gray-800 text-sm sm:text-base">{timeLeft}</span>
        ...
        <main className="container mx-auto px-4 py-4 sm:py-8">
          ...
          <div className="mb-8">
            <div 
              id="cards-container"
              className="flex gap-4 sm:gap-6 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 snap-x snap-mandatory"
              style={{
            ...
            >
              {allGames.map((game, index) => (
                <div key={index} className="flex-shrink-0 w-72 sm:w-80 snap-center">
                  <GameCard 
                    game={game} 
